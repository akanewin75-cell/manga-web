...
class ComicazenService
{
    ...
    {
        if ($query) {
            // WordPress typically handles search pagination via /page/X/?s=... or ?s=...&paged=X
            $url = $page > 1 
                ? "{$this->baseUrl}/page/{$page}/?s=" . urlencode($query) . "&post_type=wp-manga"
                : "{$this->baseUrl}/?s=" . urlencode($query) . "&post_type=wp-manga";
        } else {
            // For browsing, use the /manga/ or /komik/ archive which is more reliable for pagination
            $archivePath = ($this->mangaPath ?: '/manga');
            $url = $page > 1 
                ? "{$this->baseUrl}{$archivePath}/page/{$page}/?m_paged={$page}" 
                : "{$this->baseUrl}{$archivePath}/";
        }
        ...
        try {
            ...
            $jsonIds = ['mjv2-manga-data', 'mjv2-komik-data', 'mjv2-home-data', 'madara-manga-data'];
            if ($page > 1 && !$query) {
                // Skip home-specific data on deep pages to avoid duplicates
                $jsonIds = ['mjv2-manga-data', 'mjv2-komik-data', 'madara-manga-data'];
            }
            ...
            // Method 2: Always perform DOM Scraping as well to fill gaps
    ...
    public function getMangaDetails($slug)
    {
        ...
        try {
            ...
            // AJAX Fallback for Madara Chapters (Many sites load them via AJAX)
            if (empty($info['chapters'])) {
                \Illuminate\Support\Facades\Log::info("Comicazen: Attempting AJAX chapter fallback for $slug");
                
                // Get Manga ID from HTML (often in a data-id or script)
                $mangaId = null;
                if (preg_match('/post-(\d+)/', $html, $m)) $mangaId = $m[1];
                elseif (preg_match('/"manga_id":"(\d+)"/', $html, $m)) $mangaId = $m[1];
                elseif (preg_match('/var\s+manga_id\s*=\s*(\d+);/', $html, $m)) $mangaId = $m[1];

                if ($mangaId) {
                    $ajaxUrl = "{$this->baseUrl}/wp-admin/admin-ajax.php";
                    $ajaxRes = Http::asForm()->withHeaders($this->getHeaders("{$this->baseUrl}{$activePath}/{$slug}/"))
                        ->post($ajaxUrl, [
                    ...
                    if ($ajaxRes->successful()) {
                        ...
                        libxml_use_internal_errors(true);
                        $ajaxDom = new DOMDocument();
                        @$ajaxDom->loadHTML($ajaxHtml);
    ...
    public function getChapterImages($slug, $chapterId)
    {
        ...
        foreach ($imgNodes as $in) {
            ...
            if ($src && !str_contains($src, 'data:image') && !str_contains($src, 'loading.gif')) $images[] = trim($src);
        }
        return $images;
    }
    ...
    {
        if (preg_match('/chapter-(\d+(\.\d+)?)/i', $slug, $matches)) return $matches[1];
        return $slug;
    }
}
